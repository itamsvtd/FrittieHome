...
class FriendshipManager(models.Manager):
	...
	try:
		return bool(Friendship.objects.get(user=user1).friends.filter(user=user2).count())
    ...
    def befriend(self,user1,user2):
    	f1 = Friendship.objects.get(user=user1)	
    	f2 = Friendship.objects.get(user=user2)
    	if f2 not in f1.friends.all():
    		f1.friends.add(f2)	
    ...
    def unfriend(self, user1, user2):
        Friendship.objects.get(user=user1).friends.remove(
                                           Friendship.objects.get(user=user2))
	try:
		FriendshipRequest.objects.get(from_user=user1,to_user=user2).delete()
	except FriendshipRequest.DoesNotExist:
		pass
	try:
		FriendshipRequest.objects.get(from_user=user2,to_user=user1).delete()
	except FriendshipRequest.DoesNotExist:
		pass
...
class UserBlocks(models.Model):
    ...
    blocks = models.ManyToManyField(User, related_name='blocked_by_set',blank=True)
    ...
    def is_block(self,user_check):
        if user_check in self.blocks.all():
            return True
        else:
            return False

    def unblock(self,user_unblock):
        self.blocks.remove(user_unblock)
