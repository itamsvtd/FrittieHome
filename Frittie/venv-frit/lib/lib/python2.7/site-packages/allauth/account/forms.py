...
from frittie.app.main.models import GENDER, USA_STATES
from frittie.helper.member_helper import set_readable_username
from frittie.helper.common_helper import setup_constant_day, setup_constant_month, setup_constant_year
from frittie.settings import WEBSITE_DOMAIN
...
class LoginForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
	    ...
	    login_field = forms.CharField(label=ugettext('Email or Username'),widget=login_widget)
	    #login_widget = forms.TextInput(attrs={'placeholder': 
...
class InitSignupForm(forms.Form):
    email = forms.EmailField(widget=forms.TextInput())
    ...
    def send_activate_mail(self):
        ...
        e = EmailConfirmation.objects.filter(email_address=email)
        is_sent = True
        if len(e) == 0:
            is_sent = False
        if is_sent:
            ...
            new_email_confirm = EmailConfirmation.objects.create(email_address=email,sent=datetime.datetime.now())
...
class CompleteSignupForm(forms.Form):
    ...
    def clean(self):
        super(CompleteSignupForm, self).clean()
...
class ProfileSignupForm(forms.Form):
    ...
    def save(self,username):
        ...
        user.get_profile().basic_info = self.cleaned_data["basic_info"]
        day = int(self.cleaned_data["birthdate_day"])
        month = int(self.cleaned_data["birthdate_month"])
        year = int(self.cleaned_data["birthdate_year"])
        user.get_profile().birthdate = datetime.date(year,month,day)
        user.get_profile().avatar = self.cleaned_data["avatar"]
        user.get_profile().city = self.cleaned_data["city"]
        user.get_profile().state = self.cleaned_data["state"]
        user.save()
        return user
...
class ResetPasswordForm(forms.Form):
    ...
    def save(self, **kwargs):
        ...
        for user in self.users:
            ...
            subject = "Password Reset E-mail at Frittie"
            path = reverse("account_reset_password_from_key",
                           kwargs=dict(uidb36=int_to_base36(user.id),
                                       key=temp_key))
            #url = 'http://%s%s' % (current_site.domain,
            #                       path)
            url = 'http://%s%s' % (WEBSITE_DOMAIN,path)
            message = render_to_string \
                ("account/mail/password_reset_key_message.txt", 
                 { #"site": current_site,
                   "user": user,
